...
; tests
assert.label("with empty array")
assert.test(A.count([], 0), 0)
assert.label("with value not found in array")
assert.test(A.count([1, 2, 3], 4), 0)
assert.label("with empty string")
assert.test(A.count("", "a"), 0)
assert.label("with space character parameter")
assert.test(A.count(["a", "b", "c"], " "), 0)
assert.label("with empty string in array")
assert.test(A.count(["a", "b", "c"], ""), 0)
